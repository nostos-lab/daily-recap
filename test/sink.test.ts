...
check("link(https 허용)", renderInline("[링크](https://x.io)") === '<a href="https://x.io">링크</a>');
check("HTML escape", renderInline("<script>alert(1)</script>").includes("&lt;script&gt;"));
check("따옴표 이스케이프", renderInline(`"q" 'a'`).includes("&quot;") && renderInline(`"q" 'a'`).includes("&#39;"));
check("javascript: URL 무력화", renderInline("[x](javascript:alert(1))").includes('href="#"'));
check("mailto/anchor 허용", renderInline("[m](mailto:a@b.io)").includes('href="mailto:a@b.io"'));
