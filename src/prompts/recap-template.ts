...
/** English skeleton (used when lang === "en"). Mirrors RECAP_SKELETON. */
export const RECAP_SKELETON_EN = `# {YYYY-MM-DD} · {project} decision recap

> **One-liner**: {the single most important decision of the day, in 25 words or fewer}

<!-- Repeat the "## Topic" block below for each work thread of the day.
Group threads by coherent goal, merge when ambiguous, never invent a thread; one section if there is only one topic. -->

## Topic: {work thread name — e.g. Sign-up / UI fixes / Backend API}

**What was done** — {1-2 lines on what happened in this thread}
...
**Why decide that way? (context & judgment)**

{Each decision as a Y-Statement: "In the context of X, concerned about Y, we chose Z, accepting trade-off W."}

- **Trigger** — {short verbatim quote + source}
- **Alternatives considered** — {one bullet per alternative: "alternative — chosen/rejected because (source)". If none, "not recorded"}
...
**Anything stuck or surprising?**

{40-75 words. Wrong assumptions corrected, API gotchas, etc. If none, "not recorded".}

**What to pick up next?**

{Concrete next actions. If unclear, 1-2 open questions.}

---
*Logged: {YYYY-MM-DD} · {N} topics · {N} session turns · {N} commits · auto-generated by DailyRecap*
`;
...
11) **Segment the day into coherent work threads and repeat the "## Topic" block per thread.** Group by goal, merge when ambiguous, never invent a thread absent from the sources; one section if there is only one topic.
...
export function buildRecapPrompt(
  ...
  const skeleton = lang === "en" ? RECAP_SKELETON_EN : RECAP_SKELETON;
